  ...
  useEffect(() => {
    ...
    if (searchLower !== "") {
      const handleSearch = async () => {
        setLoading(true);
        try {
          // بدل ما تنادي API بالاسم، هات كل النباتات وفلتر بنفسك
          const response = await fetch(
            `https://localhost:7286/api/Plant?PageSize=1000`
          );
          const data = await response.json();

          if (data.isSuccess && data.value?.items) {
            const filteredItems = data.value.items.filter((plant) =>
              plant.name.toLowerCase().startsWith(searchLower)
            );
            setPlants(filteredItems);
            setHasNextPage(false); // إلغاء الباجينج وقت البحث
            setHasPrevPage(false);
            setError(filteredItems.length === 0 ? "No plant found." : "");
          } else {
            setPlants([]);
      ...
      fetchPlants(pageNumber); // لو البحث فاضي رجع للعرض العادي
    }
  }, [searchQuery, pageNumber]);
          ...
          plants.map((plant) => (
            ...
            >
              ...
              <div className="relative w-full h-48 overflow-hidden">
                ...
                >
                  {plant.is_Available ? "Available" : "Out of Stock"}
                </span>
              </div>
